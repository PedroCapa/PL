...
%token <str> obj
%token <str> item
%type <str> Programa Conteudo Estrutura Texto Top Tex Est Lista Variavel Itens Obj Indice
...
Itens: Obj Indice 						{asprintf(&$$, "%s{\n		%s\n	}", $1, $2);}
	 ;

Obj: obj 								{asprintf(&$$, "	\"%s\":", $1);}
	;

Indice: item Indice						{asprintf(&$$, "%s,	\n		%s	", $1, $2);}
	  | item 							{asprintf(&$$, "%s", $1);}
	  ;
